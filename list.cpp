...
void List::insertheadlist(int i){
  Inserthead(this->head, i);
}
void List::insertBefore(Node * beforeThis, int insertThis){
  InsertBefore(this->head, beforeThis, insertThis);
}

void List::insertAfter(Node * &afterThis, int insertThis){
  InsertAfter(this->head, afterThis, insertThis);
}

Node * List::previousNode(Node * previousToThis){
   return PreviousNode(this->head, previousToThis);
}

Node * List::whereThisGoes(int item){
  return WhereThisGoes(this->head, item);
}
...
void List::insertToList(int number){
  insert(this->head, number);
}
...
int List::deleteNode(Node * &deleteThis){
  return Delete(head, deleteThis);
}

void List::buildlink(int intArr[], int size){
  BuildLink(head, intArr, size);
}

void List::PrintList(Node * current){//head
  cout<<"H->";
  while(current!=NULL){
      cout<<*current;
...
Node * List::SearchList(Node *head, ITEM_TYPE key){
    while(head->_next != nullptr && head->_item!=key)
       head = head->_next;
    head=(head->_item!=key)?nullptr:head;
    return head;
}
void List::Inserthead(Node *&head, ITEM_TYPE insertThis){
...
Node * List::PreviousNode(Node * head, Node * previousToThis){
...
void List::InsertBefore(Node * &head, Node * beforeThis, ITEM_TYPE insertThis){
 ...
 */
void List::InsertAfter(Node * &head, Node * &afterThis, ITEM_TYPE insertThis){
 ...
 */
ITEM_TYPE List::Delete(Node * &head, Node * &deleteThis){
...
Node * List::WhereThisGoes(Node * head, ITEM_TYPE item){
  if(head->_item==item)
       return NULL;
...
*/
void List::BuildLink(Node *&head, ITEM_TYPE intArr[], ITEM_TYPE size){//** see below
    for(int i=size-1; i>=0; i--){
        insert(head, intArr[i]);
    }
}

void List::insert(Node * &head, ITEM_TYPE number){
